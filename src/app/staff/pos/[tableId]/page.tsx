...
import { addItemToOrder, markTableOccupied, reserveTable, cancelReservation, completeOrder, removeItemFromOrder, cancelOrder, applyDiscountToOrderItem } from "@/lib/pos-actions";
...
export default async function PosPage({ params }: { params: Promise<{ tableId: string }> }) {
    ...
    const serializableOrder = !openOrder
        ? null
        : {
            id: openOrder.id,
            items: (openOrder.items as any[]).map((it) => ({
                id: it.id,
                menuItemId: it.menuItemId,
                name: it.menuItem?.name || 'Item',
                quantity: it.quantity ?? 1,
                basePrice: it.menuItem ? Number(it.menuItem.price) : Number(it.unitPrice),
                unitPrice: Number(it.unitPrice),
                appliedDiscountName: (it as any).appliedDiscountName ?? null,
            })),
                ...
                <Card className="sticky top-20">
                    ...
                    <CardContent>
                        <div className="flex flex-col gap-4">
                            {!serializableOrder || serializableOrder.items.length === 0 ? (
                                ...
                                <OrderPanel
                                    ...
                                    discounts={serializableDiscounts as any}
                                    removeItemAction={removeItemFromOrder}
                                    applyDiscountAction={applyDiscountToOrderItem}
                                />
                            )}
                        </div>
                    </CardContent>
